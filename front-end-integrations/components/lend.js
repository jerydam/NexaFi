...
export default function LendSection() {
  ...
  const showMyCreatedLoans = () => {
    ...
    return (
        ...
        <div className="text-[#ccc] text-[90%]">Lend on NexaFi  to enjoy huge rewards (P2P)</div>
  ...
  <div className='text-center mt-[1cm]'>
    ...
    <form onSubmit={(e) => {e.preventDefaNEXA(); handleSearch(searchQuery)}} style={{display:"inline-block"}}>
    <input type="text" placeholder="Search by ID, lender, or borrower..." onChange={(e) => setSearchQuery(e.target.value)} className='bg-[#000] w-[6cm] placeholder-[#fff] text-[#fff] text-[90%] outline-none' /><img src="images/search.png" width="20" className='ml-[0.2cm] cursor-pointer' onClick={(e) => {e.preventDefaNEXA(); handleSearch(searchQuery)}} style={{display:"inline-block"}}/>
    </form>
    </span>
   </div>
 ...
 {searchData.length > 0 ? 
  (<div className="my-[1cm] bg-[#000] p-[0.5cm] rounded-xl" style={{boxShadow:"2px 2px 2px 2px #222"}}>
    <div className="m-[-0.5cm] mb-[0.5cm] text-center px-[0.2cm] py-[0.3cm] bg-[#222] font-[500] rounded-t-xl viewloansdiv">Search resNEXAs</div>
    ...
    {(data.collateral.toString().toLowerCase()) === tokenContractAddress.toLowerCase() && (<span> NEXA</span>)} 
    ...
    {(data.lender.toString() === "0x0000000000000000000000000000000000000000") & (data.borrower.toString() === address) & ((data.active) == true) ? (<div className="m-[0.2cm]" style={{display:"inline-block"}}><button className="text-[#fff] fa-fade bg-[#040] rounded-md px-[0.3cm] py-[0.05cm] generalbutton4" style={{boxShadow:"1px 1px 1px 2px #ccc", animationDuration:"3s"}} onClick={(e) => {e.preventDefaNEXA(); withdrawCollateral(data.loan_id.toString())}}>Withdraw collateral <img src="images/locked.png" className="ml-[0.2cm]" width="20" style={{display:"inline-block"}} /></button></div>) : (<span></span>)}
    {(data.lender.toString() != "0x0000000000000000000000000000000000000000") & (data.borrower.toString() === address) ? (<div className="m-[0.2cm]" style={{display:"inline-block"}}><button className="text-[#fff] fa-fade bg-[#040] rounded-md px-[0.3cm] py-[0.05cm] generalbutton4" style={{boxShadow:"1px 1px 1px 2px #ccc", animationDuration:"3s"}} onClick={(e) => {e.preventDefaNEXA(); withdrawLoan(data.loan_id.toString())}}>Withdraw loan <img src="images/loan.png" className="ml-[0.2cm]" width="20" style={{display:"inline-block"}} /></button></div>) : (<span></span>)}
    {(((data.active) == true) & ((data.repaid) == false) & (address === (data.lender.toString())) & ((new Date().getTime().toString()/1000) >= data.fundingDeadline.toString())) ? (<div className="m-[0.2cm]" style={{display:"inline-block"}}><button className="text-[#fff] fa-fade bg-[#040] rounded-md px-[0.3cm] py-[0.05cm] generalbutton4" style={{boxShadow:"1px 1px 1px 2px #ccc", animationDuration:"3s"}} onClick={(e) => {e.preventDefaNEXA(); claimCollateral(data.loan_id.toString())}}>Claim collateral <img src="images/locked.png" className="ml-[0.2cm]" width="20" style={{display:"inline-block"}} /></button></div>) : (<span></span>)}
    {(((data.active) == true) & ((data.repaid) == false) & (address != (data.borrower.toString())) & (data.fundingDeadline.toString() >= (new Date().getTime().toString()/1000)) & (data.lender.toString() == "0x0000000000000000000000000000000000000000")) ? (<div className="m-[0.2cm]" style={{display:"inline-block"}}><button className="text-[#fff] fa-fade bg-[#040] rounded-md px-[0.3cm] py-[0.05cm] generalbutton4" style={{boxShadow:"1px 1px 1px 2px #ccc", animationDuration:"3s"}} onClick={(e) => {e.preventDefaNEXA(); fundLoanFromLoanList(data.loan_id.toString(), parseFloat(data.amount.toString() * 10**-18).toFixed(18))}}>Fund loan <img src="images/loan.png" className="ml-[0.2cm]" width="20" style={{display:"inline-block"}} /></button></div>) : (<span></span>)}
    {(data.repaid) == false & (data.borrower.toString() === address) & (data.lender.toString() != "0x0000000000000000000000000000000000000000") ? (<div className="m-[0.2cm]" style={{display:"inline-block"}}><button className="text-[#fff] fa-fade bg-[#040] rounded-md px-[0.3cm] py-[0.05cm] generalbutton4" style={{boxShadow:"1px 1px 1px 2px #ccc", animationDuration:"3s"}} onClick={(e) => {e.preventDefaNEXA(); repayLoan(data.loan_id.toString())}}>Repay loan <img src="images/loan.png" className="ml-[0.2cm]" width="20" style={{display:"inline-block"}} /></button></div>) : (<span></span>)}
    </div>  
    </div>       
   </div>
    ))}
 ...
 {chatBox && (<div id="chatbox" className="fixed text-center top-0 p-[0.5cm] bg-[rgba(0,0,0,0)] w-[100%] right-0 rounded-xl">
      ...
      <div className="lg:px-[0.5cm] mb-[1cm]">
       <form className="bg-[#000] p-[0.2cm] rounded-xl" style={{boxShadow:"2px 2px 2px 3px #d7b644"}} onSubmit={(e) => {e.preventDefaNEXA(); sendChat(values.message, currentLender, currentBorrower, address, currentLoanID, (new Date().getTime()/1000))}}>
         ...
         <button type="submit" onClick={(e) => {e.preventDefaNEXA(); sendChat(values.message, currentLender, currentBorrower, address, currentLoanID, (new Date().getTime()/1000))}} className="ml-[0.3cm] mt-[0.5cm] absolute" style={{display:"inline-block"}}><img src="images/sendchat.png" width="30" /></button>
       </form>
      </div>
      <div className="text-right px-[1cm]"><button onClick={(e) => {e.preventDefaNEXA(); deleteChat(currentLoanID)}} className="rounded-md px-[0.5cm] pr-[1cm] py-[0.2cm] bg-[#333] text-[#fff] generalbutton4" style={{boxShadow:"2px 2px 2px 2px #111"}}>Clear chat <img src="images/bin.png" className="ml-[0.2cm] absolute" width="20" style={{display:"inline-block"}}/></button></div>
       </div>
    </div>
   </div>)}
